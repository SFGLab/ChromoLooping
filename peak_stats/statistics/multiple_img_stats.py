...
class ImageDict(MutableMapping):
    ...
    def save_stats(self, output):
        ...
        with open(outfile, 'w') as out:
            ...
            out.write("Average number of photons per spot: {}".format(self.avg_photons_per_group()) + "\n")
            out.write("Average number of peaks per image: {}".format(self.avg_peaks_per_image()) + "\n")
            out.write("Average number of peaks per group: {}".format(self.avg_peaks_per_group()) + "\n")
    ...
    def print_stats(self):
        ...
        print("Average number of photons per spot: {}".format(self.avg_photons_per_group()))
        print("Average number of peaks per image: {}".format(self.avg_peaks_per_image()))
        print("Average number of peaks per group: {}".format(self.avg_peaks_per_group()))
    ...
    def save_plots(self, output):
        ...
        data3 = self.peaks_per_group()
        self.plot_histogram(data=data3, title="Peaks per spot", xlabel="Number of peaks", ylabel="Number of spots",
                            outname=name3)

    def avg_photons_per_group(self):
        photons_per_group = 0
        for i in self.images.keys():
            photons_per_group += sum(self.images[i][1].photons_per_group) / len(self.images[i][1].photons_per_group)
        return photons_per_group / len(self)

    def photons_per_group(self):
        photons_per_spot = []
        for i in self.images.keys():
            photons_per_spot += self.images[i][1].photons_per_group
    ...
    def peaks_per_group(self):
        peaks_per_group = []
        for i in self.images.keys():
            peaks_per_group += self.images[i][1].peaks_per_group(image=i)
        return peaks_per_group

    def avg_peaks_per_group(self):
        pps = self.peaks_per_group()
        return sum(pps) / len(pps)

    def groups_per_image(self):
        groups_per_image = []
        for i in self.images.keys():
            groups_per_image.append(self.images[i][1].group_count)
