...
class ImgStats:
    ...
    def add_peak_sigma(image: Image):
        """Return peaks sigma in format[[simgax_peak1,...],[simgay_peak1...],[sigmaz_peak1]]"""
        sigma_x = []
        sigma_y = []
        sigma_z = []
        for spot in image.spots:
    ...
    def plot_sigma_x_hist(self, save=False, outdir=None):
        """Plot average X sigma for each group in image."""
    ...
    def plot_sigma_y_hist(self, save=False, outdir=None):
        """Plot average Y sigma for each group in image."""
        plt.figure(figsize=FIGSIZE)
    ...
    def plot_sigma_z_hist(self, save=False, outdir=None):
        """Plot average Z sigma for each group in image."""
        plt.figure(figsize=FIGSIZE)
    ...
    def plot_peak_sigma_x(self, x_sigma, save=False, outdir=None, draw_sigma=15):
        """Plot sigma X for each peak in image."""
        plt.figure(figsize=FIGSIZE)
    ...
    def plot_peak_sigma_y_(self, y_sigma, save=False, outdir=None, draw_sigma=15):
        """Plot sigma Y for each peak in image."""
        plt.figure(figsize=FIGSIZE)
    ...
    def plot_peak_sigma_z(self, sigma_z, save=False, outdir=None, draw_sigma=15):
        """Plot sigma Z for each peak in image."""
        plt.figure(figsize=FIGSIZE)
    ...
    def plot_average_peak_sigma(self, sigma_z, sigma_y, sigma_x, save=False, outdir=None, draw_sigma=15):
        """Plot average sigma for each peak in image."""
        avg = [0] * len(sigma_x)
        ...
        plt.title('Average Peak Sigma', fontsize=TITLEFONTSIZE)
        plt.tick_params(axis='both', which='major', labelsize=TICKFONTSIZE)
        plt.grid(True)
        if draw_sigma:
            plt.axvline(x=draw_sigma, color="red")
    ...
    def plot_average_sigma(self, save=False, outdir=None):
        """Plot average sigma for each group in image."""
        plt.figure(figsize=FIGSIZE)
    ...
    def plot_peaks_per_spot_histogram(self, image: Image, save=False, outdir=None):
        """Plot the distribution of number of peaks in each group."""
        plt.figure(figsize=FIGSIZE)
    ...
    def plot_photons_per_group(self, save=False, outdir=None):
        """Plot the distribution of number of photons in each group."""
        plt.figure(figsize=FIGSIZE)
    ...
    def plot_photons_per_peaks(self, save=False, outdir=None):
        """Plot the distribution of number of photons in each peak."""
        plt.figure(figsize=FIGSIZE)
    ...
    def print_statistics(self, image: Image, sigma_threshold=None):
        ...
        stats = "Group Count:  {}".format(self.spot_count) + "\n" + "Peak Count:   {}".format(
            self.peak_count) + "\n" + "Average peaks per group   {}".format(
            self.peak_count / self.spot_count) + "\n" + "Average sigma X:  {} nm".format(
            self.average_sigma_x()) + "\n" + "Average sigma Y:  {} nm".format(
            self.average_sigma_y()) + "\n" + "Average sigma Z:  {} nm".format(
            self.average_sigma_z()) + "\n" + "Convex hull volume:   {}".format(
            peaks.hull_volume) + "\n" + "Average number of photons per peak:  {}".format(
            sum(self.photons) / len(self.photons)) + "\n" + "Average number of photons per group peak: {}".format(
            groups.average_photons()) + "\n" + "Average sigma X per group peaks: {} nm".format(
            groups.averege_sigma_x()) + "\n" + "Average sigma Y per group peaks: {} nm".format(
            groups.averege_sigma_y()) + "\n" + "Average sigma Z per group peaks: {} nm".format(
            groups.averege_sigma_z())
        print(stats)

    def save_statistics(self, output, image: Image, sigma_threshold):
        """Save basic image statistics in text file."""
    ...
    def photons_in_image(image: Image):
        """Return a list of number of photons in each peak in the image"""
        photons_per_peak = []
        for spot in image.spots:
